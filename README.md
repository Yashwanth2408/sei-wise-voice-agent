    ...
    Sei Wise Voice Agent is a scoped voice support demo for Wise transfer-tracking questions. It listens to the caller over LiveKit, transcribes speech in real time, routes the request through a Wise-specific FAQ scope, and speaks a grounded answer back with TTS.
    ...
    1. The browser captures microphone audio and sends it to the backend through LiveKit.
    ...
    - `POST /livekit/session` creates a LiveKit room, starts the agent, and returns the browser join token.
    ...
    - Establishes the LiveKit session
    ...
    - If the browser cannot connect, confirm the LiveKit URL and API credentials are configured.
